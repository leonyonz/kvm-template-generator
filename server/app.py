...
PANEL_MODES = {
    "panel-cpanel": {
        ...
        "hints": [
            "WHM web UI : https://<server-ip>:2087  (login: root)",
            "NOTE       : the server IP needs a valid cPanel license",
        ],
        "script": """set -x
cd /home
...
""",
    ...
    "panel-plesk": {
        ...
        "hints": [
            ...
            "Next step  : run 'plesk login' as root for a one-time login link",
        ],
        "script": """set -x
...
""",
    ...
    "panel-aapanel": {
        ...
        "hints": [
            "Next step  : run 'bt default' as root for panel URL + password",
        ],
        "script": """set -x
...
""",
    ...
    "panel-cyberpanel": {
        ...
        "hints": [
            "Web UI     : https://<server-ip>:8090  (admin password set by installer)",
        ],
...
""",
    ...
    "panel-hermes": {
        ...
        "hints": [
            "Setup      : run 'hermes' to launch the agent",
            "Config     : ~/.hermes/   Binary: /usr/local/bin/hermes",
...
""",
    ...
    "panel-openclaw": {
        ...
        "hints": [
            "Setup      : run 'openclaw onboard --install-daemon' to finish setup",
            "Config     : ~/.openclaw   Docs: https://docs.openclaw.ai",
        ],
        "script": """set -x
...
def _panel_welcome(label: str, hints: list[str]) -> str:
    """Login-shell banner showing live panel-install status + next steps."""
    hint_block = "\n".join(f'  echo "{h}"' for h in hints) or '  echo ""'
    # NOTE: this file lands in /etc/profile.d/ and is SOURCED by the login
    # shell — an `exit` here kills the whole session (SSH "closes
    # immediately"). Only ever `return` from this snippet.
    return f'''# Auto-generated by template-generator - {label}
# sourced from /etc/profile.d — must never call exit
case "$-" in *i*) ;; *) return 0 ;; esac
LOG=/var/log/tg-panel-install.log
[ -f "$LOG" ] || return 0
st=$(systemctl is-active tg-panel-install.service 2>/dev/null)
echo "=============================================================="
echo " template-generator : {label}"
case "$st" in
  activating) echo " status : INSTALLER STILL RUNNING - give it a few minutes" ;;
  failed)     echo " status : INSTALL FAILED - check: tail -50 $LOG" ;;
...
def resolve_build(req: "BuildRequest") -> dict:
    ...
    if req.patch_mode in PANEL_MODES:
        p = PANEL_MODES[req.patch_mode]
        ev["panel_label"] = p["label"]
        ev["panel_script"] = p["script"]
        ev["panel_unit"] = PANEL_UNIT
        ev["panel_welcome"] = _panel_welcome(p["label"], p.get("hints", []))
